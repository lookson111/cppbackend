...
CollectionResult TryCollectPoint(geom::Point2D a, geom::Point2D b, geom::Point2D c) {
    ...
    const auto u_x = c.x - a.x;
    const auto u_y = c.y - a.y;
    const auto v_x = b.x - a.x;
    const auto v_y = b.y - a.y;
    const auto u_dot_v = u_x * v_x + u_y * v_y;
    const auto u_len2 = u_x * u_x + u_y * u_y;
    const auto v_len2 = v_x * v_x + v_y * v_y;
    const auto proj_ratio = u_dot_v / v_len2;
    const auto sq_distance = u_len2 - (u_dot_v * u_dot_v) / v_len2;
